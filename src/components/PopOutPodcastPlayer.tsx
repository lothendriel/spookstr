...
export function PopOutPodcastPlayer() {
  ...
  const handleMouseMove = (e: MouseEvent) => {
    if (isDragging) {
      ...
      const maxX = window.innerWidth - 350; // 350px is approximate width
      const maxY = window.innerHeight - 300; // 300px is approximate height
    ...
    >
      <Card className="border-0 bg-black/95 backdrop-blur-md">
        ...
        <CardContent className="p-0">
          <div
            dangerouslySetInnerHTML={{ __html: currentPodcast.embedCode.replace('height="400"', 'height="200"') }}
          />
        </CardContent>
      </Card>
    </div>
