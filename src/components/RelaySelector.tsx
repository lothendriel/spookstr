...
export function RelaySelector(props: RelaySelectorProps) {
  ...
  const { config, updateConfig, presetRelays = [], computedRelays = [], useProfileRelays, setUseProfileRelays } = useAppContext();
  ...
  // Create options from computed relays
  const relayOptions = computedRelays.map(url => {
    const preset = presetRelays.find(p => p.url === url);
    return {
      url,
      name: preset?.name || url.replace(/^wss?:\/\//, '').replace(/^https?:\/\//, '')
    };
  });

  const selectedOption = relayOptions.find((option) => option.url === selectedRelay);
  ...
  return (
    <div className="space-y-2">
      {/* Profile relays toggle */}
      <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
        <Label htmlFor="profile-relays" className="text-sm font-medium">
          Use Profile Relays
        </Label>
        <Switch
          id="profile-relays"
          checked={useProfileRelays}
          onCheckedChange={setUseProfileRelays}
        />
      </div>
