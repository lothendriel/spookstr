...
const activeSubscriptions = new Map<string, { count: number; closer: (() => void) | null }>();
...
export function useRealtimeInteractionUpdates(eventIds: string[]) {
  ...
  useEffect(() => {
    ...
    if (subInfo) {
      ...
      const closer = createSharedSubscription(
        nostr,
        eventIds,
        queryClient,
        updateQueueRef,
...
function createSharedSubscription(
  ...
  try {
    // Create a single subscription for all interaction types on these events
    const sub = nostr.req([{
    ...
    // Batch flush updates every 2 seconds
